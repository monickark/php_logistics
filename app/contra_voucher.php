...
/*        "pay_mode"         =>     mysqli_real_escape_string($data->con, $_POST['pay_mode']),*/
 ...
 $bank_insert = $data->insert('contra_entries', $bank);
...
/*  $bank_insert= "insert into banks(name,accno,ifsc,subgroup,state,city,area,address,pincode)
  values('".$bnkname."','".$accno."','".$ifsc."','".$subgrp."','".$state."','".$city."','".$area."','".$address."','".$pincode."')";  */ 
   ...
   if($bank_insert){
    ...
    echo '<script> alert("Voucher Created Successfully");window.location.assign("contra_voucher.php");</script>';
   } 
   else{
...
else
{
 ...
 $update_id = array(
      "id"      =>    mysqli_real_escape_string($data->con, $id)
       ...
       $insid = $data->update('contra_entries', $bank, $update_id);
      if($insid){
 ...
 $id =$_REQUEST['id'];
    ...
    $insid = $data->delete_where('contra_entries', $update_id);
echo '<script> alert("Voucher Deleted Successfully");window.location.assign("recipt_voucher.php");</script>';
 ...
 $id =$_REQUEST['id'];
    ...
    $insid = $data->select_where('contra_entries', $update_id);
 }
       ...
       <div class="row row-sm mg-t-20">
          <div class="col-xl-6">
            ...
            <div class="portlet-body">
                                          <h6 class="card-body-title">Contra Entry</h6>
...
<?php
 $pinqry = mysqli_query($conn,"SELECT id FROM contra_entries ORDER BY id DESC LIMIT 1 "); 
...
$group = mysqli_query($conn,"SELECT * FROM ledger WHERE under ='".$row['comp_id']."' AND  nature='Bank'  "); 

}
else
{

 $group = mysqli_query($conn,"SELECT * FROM ledger WHERE under =''"); 
}
...
$bnk_name =$nme[0];
                                                            ...
                                                            ?>
                                                              <option value="<?php echo $grp['name'];?>" <?php if($grp['name']==$insid[0]['from_ledger']) echo 'selected="selected"'; ?>><?php echo $bnk_name;?></option>
                                                              <?php
 ...
 <div class="row mg-t-10">
                                                        ...
                                                        <div class="col-sm-8 mg-t-10 mg-sm-t-0">
                                                              ...
                                                              <select class="form-control" name="ledger_id_to" id="ledger_id_to" onchange="bankdet(this.value)" <?php if($_REQUEST['flag'] == "Edit"){
                                                                echo "disabled";
                                                              } ?>>
                                                                <option value="">Select Ledger</option>
                                                            <?php
...
$bnk_name =$nme[0];
                                                            ...
                                                            ?>
                                                              <option value="<?php echo $grp['name'];?>" <?php if($grp['name']==$insid[0]['to_ledger']) echo 'selected="selected"'; ?>><?php echo $bnk_name;?></option>
                                                              <?php
