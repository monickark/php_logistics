...
/*        "pay_mode"         =>     mysqli_real_escape_string($data->con, $_POST['pay_mode']),*/
 ...
 $bank_insert = $data->insert('payment_voucher', $bank);
...
/*  $bank_insert= "insert into banks(name,accno,ifsc,subgroup,state,city,area,address,pincode)
  values('".$bnkname."','".$accno."','".$ifsc."','".$subgrp."','".$state."','".$city."','".$area."','".$address."','".$pincode."')";  */ 
   ...
   if($bank_insert){
    ...
    echo '<script> alert("Voucher Created Successfully");window.location.assign("payment_voucher.php");</script>';
   } 
   else{
...
else
{
 ...
 $update_id = array(
      "id"      =>    mysqli_real_escape_string($data->con, $id)
       ...
       $insid = $data->update('payment_voucher', $bank, $update_id);
      if($insid){
 ...
 $id =$_REQUEST['id'];
    ...
    $insid = $data->delete_where('payment_voucher', $update_id);
echo '<script> alert("Voucher Deleted Successfully");window.location.assign("view_voucher.php");</script>';
 ...
 $id =$_REQUEST['id'];
    ...
    $insid = $data->select_where('payment_voucher', $update_id);
 }
       ...
       <div class="row row-sm mg-t-20">
          <div class="col-xl-6">
            ...
            <div class="portlet-body">
                                          <h6 class="card-body-title">Payment Voucher</h6>
...
<?php
 $pinqry = mysqli_query($conn,"SELECT id FROM payment_voucher ORDER BY id DESC LIMIT 1 "); 
...
/*$group = mysqli_query($conn,"SELECT * FROM ledger WHERE under ='Transporter'"); */
...
if($_REQUEST['flag'] == "Edit")
{
  ...
  if( $insid[0]['pay_type'] !="Cash")
{
?>
<input type="hidden" id="flag_type" name="flag_type" value="<?php echo $_REQUEST['flag']; ?>">
 ...
 <select class="form-control" data-val="true" data-val-number="The field maritialstatus must be a number." placeholder="The maritialstatus field is required." id="bank_det"  name="bank_det" <?php if($_REQUEST['flag'] == "Edit"){
    ...
    <div class="row mg-t-10" id="cque_div">
      ...
      <div class="row mg-t-10" id="paid_bank_div">
       <label class="col-md-4 form-control-label">Paid From</label>
       <div class="col-sm-8 mg-t-10 mg-sm-t-0">
<!--         <input class="form-control" data-val="true" data-val-length="The field insurancename must be a string with a maximum length of 200." data-val-length-max="200" placeholder="Enter Bank Name" id="frt_adv_cqe_bnk" maxlength="200" name="frt_adv_cqe_bnk" type="text" value=""> -->
...
{
  $mod=$insid[0]['id'];
$acct=mysqli_query($conn,"SELECT * from account_voucher where pay_mode ='Payment Voucher' and mod_voucher_no='$mod';"); 
$act = mysqli_fetch_assoc($acct);
...
$bnk_name =$nme[0];
  ...
  ?>
            <option  value="<?php echo $bnk['name'];?>"  <?php if($bnk['name']==$act['ledger_id']) echo 'selected="selected"'; ?>><?php echo $bnk_name;?></option>
